...
export function NewNoteCard({ onNoteCreated } : NoteCardProps) {
  ...
  function handleSaveNote(e: FormEvent) {
    ...
    if (!content)
      return toast.error('Informe uma anotação para salvar!')
  ...
  function handleStartRecording(e: FormEvent<HTMLButtonElement>) {
    e.preventDefault();
    ...
    const SpeechRegnitionAPI = window.SpeechRecognition || window.webkitSpeechRecognition

    speechRecognition = new SpeechRegnitionAPI()
  ...
  function handleStopRecording(e: FormEvent<HTMLButtonElement>) {
    e.preventDefault();
          ...
          >
            <div className="flex flex-1 flex-col gap-3 p-5">
              ...
              {shouldShowOnboarding ?
                <p className='text-sm leading-6 text-slate-400'>
                  Comece <button className='font-medium text-lime-400 hover:underline' onClick={handleStartRecording}>gravando uma nota</button> em áudio ou se preferir <button className='font-medium text-lime-400 hover:underline' onClick={handleStartEditor}>utilize apenas texto</button>.
                </p>
              : <textarea 
                  autoFocus
